...
async function loadData(){try{const r=await fetch('api/data.php');const d=await r.json();
 const c=document.getElementById('contacts');if(c)c.innerHTML=(Array.isArray(d.contacts)?d.contacts:[]).map(x=>`<div class="contact-row"><div class="row-main">${x.logo ? `<img class="contact-logo" src="uploads/logos/${encodeURIComponent(x.logo.split('/').pop())}" alt="${esc(x.name)} logo" onerror="this.remove()">` : '<div class="row-icon">☎</div>'}<div><b>${esc(x.name)}</b><small>${esc(x.position||'')}<br>${esc(x.phone)}</small></div></div><a class="call-btn" href="tel:${esc(x.phone)}">☎</a></div>`).join('');
 const a=document.getElementById('alertsList');if(a)a.innerHTML=(Array.isArray(d.alerts)?d.alerts:[]).map(x=>`<div class="alert ${esc(x.type)}"><b>⚠ ${esc(x.title)}</b><p>${esc(x.message)}</p><small>${esc(x.created_at)}</small></div>`).join('');
 const h=document.getElementById('hospitalsList');if(h)h.innerHTML=(Array.isArray(d.hospitals)?d.hospitals:[]).map(x=>`<div class="hospital-row"><div class="row-main"><div class="row-icon">🏥</div><div><b>${esc(x.name)}</b><small>${esc(x.location)}<br>${esc(x.phone||'')}</small></div></div><a class="call-btn" href="tel:${esc(x.phone||'')}">☎</a></div>`).join('');
 const e=document.getElementById('evacuationList');if(e)e.innerHTML=(Array.isArray(d.evacuation)?d.evacuation:[]).map(x=>`<div class="evac-row"><div class="row-main"><div class="row-icon">🏠</div><div><b>${esc(x.name)}</b><small>${esc(x.location)}<br>Capacity: ${esc(x.capacity)}</small></div></div><span class="status ${String(x.status||'').toLowerCase()}">${esc(x.status)}</span></div>`).join('');
 const f=document.getElementById('firstaidList');if(f)f.innerHTML=(Array.isArray(d.first_aid)?d.first_aid:[]).map(x=>`<details class="aid-row"><summary><div class="row-main"><div class="row-icon">🩺</div><div><b>${esc(x.title)}</b><small>Tap to view first aid instructions</small></div></div><span class="aid-arrow">⌄</span></summary><div class="aid-content">${esc(x.description).replace(/\n/g,'<br>')}</div></details>`).join('');
